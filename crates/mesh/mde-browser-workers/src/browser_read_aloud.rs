...
pub struct BrowserReadAloudWorker {
    ...
    /// Exact-body capability verifier for TTS command execution.
            ...
            match parse_request(&body, &msg.ulid) {
                Ok(request) => {
                    if let Err(e) = self.authorizer.authorize(
                        &body,
                        MutationContext {
                            verb: "browser-read-aloud",
                            node: &self.node,
                            target: "browser-read-aloud",
                        },
                    ) {
...
mod tests {
    ...
    #[test]
    fn read_aloud_mutations_require_authorization_and_replay_is_rejected() {
        use crate::browser_policy::ipc::action_auth::{
            authorize_test_body, ActionAuthorizer, MutationContext,
        };
        let tmp = tempfile::tempdir().unwrap();
        let key = b"browser-read-aloud-auth-test-key";
        let context = MutationContext {
            verb: "browser-read-aloud",
            node: "node-a",
            target: "browser-read-aloud",
        };
        let unsigned = request("Read this page.");
